...
namespace GameOfLife
{
    ...
    public class LifeSimulation
    {
        ...
        private void Grow()
        {
            for (int i = 0; i < _heigth; i++)
            {
                for (int j = 0; j < _width; j++)
                {
                    ...
                    //Если есть свободное место, то клетка пережившая цикл делится в него
        ...
        private int GetNeighbors(int x, int y)
        {
            ...
            //Если клетка находится у границ чашки то считаем что у нее есть сосед
            if (((x + 1 > _heigth) || (x - 1 < 0)) || ((y + 1 > _heigth) || (y - 1 < 0)))
            {
                NumOfAliveNeighbors++;
            }
        ...
        private void FindPlaceToGrow(int x, int y)
        {
            for (int i = x - 1; i < x + 2; i++)
            {
                for (int j = y - 1; j < y + 2; j++)
